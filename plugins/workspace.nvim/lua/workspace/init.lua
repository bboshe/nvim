...
function WorkspaceManager.load_workspace(path, verbose)
    ...
    if ws:valid() then
        print("loading workspace " .. self.storage_path.filename)
        ws:load()
    else
        if verbose then
            vim.api.nvim_echo({{"No saved workspace for "..ws_dir, "RedText"}}, true, {})
        end
    end
end
...
function WorkspaceManager.setup ()
    ...
    vim.api.nvim_create_user_command("WorkspaceLoad"  , WorkspaceManager.load_workspace,   {})
